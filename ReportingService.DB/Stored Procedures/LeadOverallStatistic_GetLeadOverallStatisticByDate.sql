...
BEGIN
  ...
  left join 
    (select DISTINCT LeadId, count(Amount) as 'DepositsCount', sum(Amount * Rate) as 'DepositsSum' from [dbo].[Transaction] as t
  ...
  left join 
    (select DISTINCT LeadId, count(Amount) as 'WithdrawalsCount', sum(Amount * Rate) as 'WithdrawalsSum' from [dbo].[Transaction] as t
  ...
  left join 
    (select DISTINCT LeadId, count(Amount) as 'TransfersCount', sum(Amount* Rate) as 'TransfersSum' from [dbo].[Transaction] as t
